...
class AdminController extends Controller
{
    ...
    public function viewCadastroEspecificacoes()
    {
        ...
        return view('admin.cadastroEspecificacoes', [
    ...
    public function cadastroModelo(Request $request)
    {
        ...
        $modelo_id = Modelo::where('nome_modelo', '=', $request->nome_modelo)
            ->get()
            ->pluck('id')
            ->first();
        $imagens = new imagem();
        $imagens->modelo_id = $modelo_id;
        $imagens->imagem_card = $request->imagem_card->store('modelos');
        if ($request->imagem_1) {
            $imagens->imagem_1 = $request->imagem_1->store('modelos');
        }
        if ($request->imagem_2) {
            $imagens->imagem_2 = $request->imagem_2->store('modelos');
        }
        if ($request->imagem_3) {
            $imagens->imagem_3 = $request->imagem_3->store('modelos');
        }
        if ($request->imagem_4) {
            $imagens->imagem_4 = $request->imagem_4->store('modelos');
        }
        $imagens->save();
        return redirect()->route('admin.cadastroEspecificacoes');
    }
